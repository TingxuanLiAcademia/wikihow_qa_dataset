...
def convert_summary(data, index_main_title):
    ...
    for idx, content in enumerate(contents):
        ...
        for sub_idx, c in enumerate(part_contents):
            # bold line が空の場合、対応する記事も空にする
            # 逆も然り
            temp_dict = {
            "qid":index_main_title + f'{str(idx).zfill(2)}'+f'{str(sub_idx).zfill(2)}',
            "question":"",
            "context":"",
            ...
            if article != '' and bold_line != '':
                ...
                bold_line = normal(bold_line)
                temp_dict['context'] = remecab(article)
                temp_dict['answers']['text'] = remecab(bold_line)
